...
use OMP_LIB
    ...
    !------------------!!!!
    implicit none
    integer,parameter::k_pn=2,nspin=2
...
do k=1,k_pn**3
   ...
   ! for the each element of lpp and lphd(lphc)
     do oo1=1,nspin
        do oo2=1,nspin
            do oo3=1,nspin
                do oo4=1,nspin
...
! the variation of V 
vert=vert0-(-pp+phd-phc)*dlambda/(k_pn**3)
call cpu_time(finish)
print*,"total time:",finish-start,"s"

 if(maxval(abs(vert))<vertmax) then
    vert0=vert
    print*,"flow",step,lambda,maxval(real(vert)),minval(real(vert))
    step=step+1
    lambda=lambda-dlambda
  goto 20
else
  print*,"divergency:",lambda,maxval(real(vert)),minval(real(vert))
endif
...
!----------calculate bare energy and its eigenvectors----------------!
...
subroutine lpp_cal(k_pn,k_point,k1,k2,k,o1,o2,o3,o4,lambda,lpp)
    ...
    if(valk(i)>0.0.and.valkindex(j)<=0.0) then
        ...
        lppfre=lppfre/(valk(i)+valkindex(j))
...
subroutine lph_cal(k_pn,k_point,k1,k3,k,o1,o2,o3,o4,lambda,lph)
    ...
    if(valk(i)>0.0.and.valkindex(j)<=0.0) then
        ...
        lphfre=lphfre/(valk(i)-valkindex(j))
            ...
            lphfre=valk(i)*(valk(i)*3.0-lambda)/((valk(i)-lambda)**3)-&
                  &valkindex(j)*(valkindex(j)*3.0+lambda)/((valkindex(j)+lambda)**3)
            lphfre=lphfre/(valk(i)-valkindex(j)+cmplx(0.0,delta))
        else
